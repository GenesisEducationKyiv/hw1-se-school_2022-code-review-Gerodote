...
class main_app():
    ...
    def __init__(self):    
        ...
        self._binance_websocket = GetterPriceBinance.book_ticker_price_binance(symbol="BTCUAH")
        self._file_with_emails = "subscribed_emails.json"
        return
    
    # def __del__(self):
    #     del(self._binance_websocket)
    def stop_binance_websocket(self):
        self._binance_websocket.stop_ws()
    
    async def get_rate(self) -> float:
        task_get_price = asyncio.create_task(self._binance_websocket.get_price("BTCUAH"))
        ...
        if os.path.exists(self._file_with_emails):
            async with aiofiles.open(self._file_with_emails, 'r') as subscribed_emails:
                ...
                try:
                    ...
                    copy_destination = ('.'.join(((str(self._file_with_emails)).split('.'))[:-1])) +\
                        "__saved." + \
                        str(self._file_with_emails).split('.')[-1]
                    print(f"!!! File subscribed_emails is broken. Copying it to {copy_destination}")
