...
class WebsocketStreamsReceiver():
    ...
    def __call__(self, message:Dict):
        ...
        if 'result' in message.keys():
            ...
            if self.__key_in_message_corresponding_to_stream_name is not None:
                if message[self.__key_in_message_corresponding_to_stream_name] not in self.__connected_streams:
                    self.__connected_streams.append(message[self.__key_in_message_corresponding_to_stream_name])
                self.__dict_connected_streams_time.update({message[self.__key_in_message_corresponding_to_stream_name]:time()})
            else:
    ...
    def get_info_about_connected_streams(cls) -> str:
        ...
        for key_symbol in cls.__dict_connected_streams_time:
            if time_now - cls.__dict_connected_streams_time[key_symbol] < cls.delta_time:
